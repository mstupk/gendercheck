...
def process_xml(input_file, output_handles, timeout):
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    # Extract source and target strings
                    ...
                    src_string = src_elem.text or ''
                    trg_string = trg_elem.text or ''

                    # Clean and validate strings
                    src_clean = src_string.strip().replace('\n', ' ').replace('\r', '')
                    trg_clean = trg_string.strip().replace('\n', ' ').replace('\r', '')

                    if not src_clean or not trg_clean:
                        continue

                    # Create unique key for this entry
                    entry_key = (path, pattern, src_clean, trg_clean)
    ...
    print(f"Processing complete. Total entries processed: {entry_count}")
