...
def process_html_file(file_path, regexes):
    """Process a single HTML file and return matches for all regexes along with article text"""
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            return matches, text  # Return both matches and full article text
    except Exception as e:
        print(f"Error processing {file_path}: {str(e)}")
        return [], None

def write_xml_entry(xml_file, file_path, matches, article_text):
    """Write matches and article text for a single file to the XML file"""
    if not matches:
        return
    ...
    # Add article text as a child element
    article_elem = ET.SubElement(entry, 'article')
    article_elem.text = article_text
        ...
        for dirpath, _, filenames in os.walk(root_dir):
            for filename in filenames:
                if filename.lower().endswith('.html'):
                    ...
                    matches, article_text = process_html_file(full_path, regexes)  # Get both values
                    if matches:  # Only write entries with matches
                        write_xml_entry(f, full_path, matches, article_text)
